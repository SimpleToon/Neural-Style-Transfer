...
function Modal({ children, modalState, close, title }) {
    ...
    return (
        <aside className={`fixed left-0 top-0 bg-black/60 w-screen h-screen flex justify-center items-center z-20 ${modalState ? "" : "hidden"}`} onClick={close}>
            {/* Center Modal */}
            <div className="w-1/2 h-1/2 bg-white z-20 flex flex-col justify-center items-center p-2 relative" onClick={(e) => e.stopPropagation()}>
                {/*Header*/}
